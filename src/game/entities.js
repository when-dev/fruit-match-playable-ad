...
export function createFruit({ speedMultiplier = 1 } = {}) {
	...
	return {
		...
		size,
		speed:
			randomBetween(GAME_CONFIG.fruit.minSpeed, GAME_CONFIG.fruit.maxSpeed) *
			speedMultiplier,
		points: GAME_CONFIG.fruit.points,
	}
}

export function createBomb({ speedMultiplier = 1 } = {}) {
	...
	return {
		...
		y: -size,
		size,
		speed: GAME_CONFIG.bomb.speed * speedMultiplier,
		damage: GAME_CONFIG.bomb.damage,
	}
}

export function createRandomEntity({ bombChance, speedMultiplier }) {
	const shouldCreateBomb = Math.random() < bombChance

	if (shouldCreateBomb) {
		return createBomb({ speedMultiplier })
	}

	return createFruit({ speedMultiplier })
}
